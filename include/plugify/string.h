...
namespace plg {
	...
	private:
		...
		constexpr void set_short_size(size_type size) noexcept {
			storage._short.size.spare_size = min_cap - (size & 0x7F);
		}

		constexpr size_type get_short_size() const noexcept {
			return min_cap - storage._short.size.spare_size;
		}
		...
		constexpr void set_long_data(value_type* data) {
			storage._long.data = data;
		}

		constexpr void is_long(bool l)  noexcept {
			storage._long.is_long = l;
		}

		constexpr bool is_long() const noexcept {
			return storage._long.is_long == true;
		}
	...
	public:
		explicit constexpr basic_string(detail::uninitialized_size_tag, size_type size, const allocator_type& alloc)
				requires(detail::is_allocator_v<Allocator>) : _allocator(alloc)
		{
		...
		constexpr basic_string() noexcept(std::is_nothrow_default_constructible<allocator_type>::value)
			requires(detail::is_allocator_v<Allocator>) : basic_string(allocator_type())
		{}
		...
		constexpr basic_string(const basic_string& str, size_type pos, const allocator_type& alloc = allocator_type())
			requires(detail::is_allocator_v<Allocator>) : basic_string(str, pos, npos, alloc)
		{}
		...
		constexpr basic_string(const value_type* str, const allocator_type& alloc = allocator_type())
			requires(detail::is_allocator_v<Allocator>) : basic_string(str, Traits::length(str), alloc)
		{}
		...
		constexpr basic_string(basic_string&& str) noexcept(std::is_nothrow_move_constructible<allocator_type>::value)
			: storage(str.storage), _allocator(std::move(str._allocator))
		{
			str.short_init();
		}

		constexpr basic_string(std::initializer_list<value_type> il, const allocator_type& alloc = allocator_type())
			requires(detail::is_allocator_v<Allocator>) : _allocator(alloc)
		{
			auto len = il.size();
			_PLUGIFY_STRING_ASSERT(len <= max_size(), "plg::basic_string::basic_string(): constructed string size would exceed max_size()", std::length_error);
			internal_assign(const_pointer(il.begin()), len);
		}
		...
		constexpr basic_string(basic_string&& str, size_type pos, size_type count, const allocator_type& alloc = allocator_type())
			requires(detail::is_allocator_v<Allocator>) : basic_string(std::move(str), alloc)
		{
			_PLUGIFY_STRING_ASSERT(pos <= str.get_size(), "plg::basic_string::basic_string(): pos out of range", std::out_of_range);
			erase(pos, count);
        }
		...
		constexpr basic_string(basic_string&& str, size_type pos, const allocator_type& alloc = allocator_type())
			requires(detail::is_allocator_v<Allocator>) : basic_string(std::move(str), pos, npos, alloc)
		{}
		...
		[[nodiscard]] constexpr reference at(size_type pos) {
			_PLUGIFY_STRING_ASSERT(pos < get_size(), "plg::basic_string::at(): pos out of range", std::out_of_range);
			return get_data()[pos];
		}
		...
		constexpr void reserve(size_type cap) {
			...
			auto new_cap = std::max(cap, get_size());
		...
		constexpr basic_string& insert(size_type pos, size_type count, value_type ch) {
			...
			_PLUGIFY_STRING_ASSERT(pos <= get_size(), "plg::basic_string::insert(): pos out of range", std::out_of_range);
		...
		constexpr iterator insert(const_iterator pos, std::initializer_list<value_type> il) {
			_PLUGIFY_STRING_ASSERT(get_size() + il.size() <= max_size(), "plg::basic_string::insert(): resulted string size would exceed max_size()", std::length_error);
			auto spos = std::distance(cbegin(), pos);
			internal_insert(spos, const_pointer(il.begin()), il.size());
			return std::next(begin(), spos);
		}
		...
		constexpr basic_string& append(std::initializer_list<value_type> il) {
			_PLUGIFY_STRING_ASSERT(get_size() + il.size() <= max_size(), "plg::basic_string::append(): resulted string size would exceed max_size()", std::length_error);
			internal_append(const_pointer(il.begin()), il.size());
			return *this;
		}
		...
		constexpr basic_string& replace(size_type pos, size_type count, const basic_string& str) {
			_PLUGIFY_STRING_ASSERT(pos <= get_size(), "plg::basic_string::replace(): pos out of range", std::out_of_range);
		...
		constexpr basic_string& replace(const_iterator first, const_iterator last, std::initializer_list<value_type> il) {
			return replace(first, last, const_pointer(il.begin()), il.size());
		}
		...
		[[nodiscard]] constexpr basic_string substr(size_type pos = 0, size_type count = npos) const {
			_PLUGIFY_STRING_ASSERT(pos <= get_size(), "plg::basic_string::substr(): pos out of range", std::out_of_range);
			return basic_string(*this, pos, count);
		}

		constexpr size_type copy(value_type* str, size_type count, size_type pos = 0) const {
			_PLUGIFY_STRING_ASSERT(pos <= get_size(), "plg::basic_string::copy(): pos out of range", std::out_of_range);
		...
		[[nodiscard]] friend constexpr basic_string operator+(const basic_string& lhs, const basic_string& rhs) {
			...
			basic_string ret(detail::uninitialized_size_tag(), lhs_sz + rhs_sz, basic_string::allocator_traits::select_on_container_copy_construction(lhs.get_allocator()));
		...
		[[nodiscard]] friend constexpr basic_string operator+(const Char* lhs, const basic_string& rhs) {
			...
			auto rhs_sz = rhs.size();
			basic_string ret(detail::uninitialized_size_tag(), lhs_sz + rhs_sz, basic_string::allocator_traits::select_on_container_copy_construction(rhs.get_allocator()));
			auto buffer = ret.get_data();
		...
		[[nodiscard]] friend constexpr basic_string operator+(Char lhs, const basic_string& rhs) {
			auto rhs_sz = rhs.size();
			basic_string ret(detail::uninitialized_size_tag(), rhs_sz + 1, basic_string::allocator_traits::select_on_container_copy_construction(rhs.get_allocator()));
			auto buffer = ret.get_data();
		...
		[[nodiscard]] friend constexpr basic_string operator+(const basic_string& lhs, Char rhs) {
			auto lhs_sz = lhs.size();
			basic_string ret(detail::uninitialized_size_tag(), lhs_sz + 1, basic_string::allocator_traits::select_on_container_copy_construction(lhs.get_allocator()));
			auto buffer = ret.get_data();
			Traits::copy(buffer, lhs.data(), lhs_sz);
	...
	namespace detail {
		...
		_PLUGIFY_STRING_ALWAYS_INLINE S to_string(V v) {
		...
		_PLUGIFY_STRING_ALWAYS_INLINE S as_string(P sprintf_like, const typename S::value_type* fmt, V v) {
