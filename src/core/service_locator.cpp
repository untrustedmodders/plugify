...
struct ServiceLocator::Impl {
    ...
    void RegisterFactory(std::type_index type,
                       std::function<std::shared_ptr<void>()> factory,
                       ServiceLifetime lifetime) {
        std::unique_lock lock(mutex);
    ...
    std::shared_ptr<void> Resolve(std::type_index type) const {
        ...
        if (it == services.end()) {
            throw std::runtime_error(
                std::format("Service not registered: {}", type.name())
            );
        }
...
void ServiceLocator::RegisterInstanceInternal(std::type_index type,
                                              std::shared_ptr<void> instance) {
    _impl->RegisterInstance(type, std::move(instance));
}

void ServiceLocator::RegisterFactoryInternal(std::type_index type,
                                            std::function<std::shared_ptr<void>()> factory,
                                            ServiceLifetime lifetime) {
