...
Result<std::string> StandardFileSystem::ReadTextFile(const std::filesystem::path& path) {
    ...
    if (!file) {
        return std::unexpected(GetStreamError(path, "Failed to open file for reading"));
    }
    ...
    if (size == -1) {
        return std::unexpected(GetStreamError(path, "Failed to determine file size"));
    }
    ...
    if (!file) {
        return std::unexpected(GetStreamError(path, "Failed to read file content"));
    }
...
Result<void> StandardFileSystem::WriteTextFile(const std::filesystem::path& path, std::string_view content) {
    ...
    if (!parent.empty()) {
        ...
        if (ec) {
            return std::unexpected(std::format("Failed to create parent directories: {}", ec.message()));
        }
    }
    ...
    if (!file) {
        return std::unexpected(GetStreamError(path, "Failed to write to file"));
    }
...
Result<FileInfo> StandardFileSystem::GetFileInfo(const std::filesystem::path& path) {
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to get file status: {} - {}",
...
Result<std::vector<FileInfo>> StandardFileSystem::ListDirectory(const std::filesystem::path& directory) {
    if (!IsDirectory(directory)) {
        return std::unexpected(std::format("Path is not a directory: {}", directory.string()));
    }
    ...
    if (options.recursive) {
        ...
        for (auto it = std::filesystem::recursive_directory_iterator(directory, dir_options, ec);
            ...
            if (ec) {
                ...
                return std::unexpected(std::format("Error iterating directory recursively: {}", ec.message()));
            }
        ...
        for (auto it = std::filesystem::directory_iterator(directory, ec);
            ...
            if (ec) {
                ...
                return std::unexpected(std::format("Error iterating directory: {}", ec.message()));
            }
...
Result<void> StandardFileSystem::RemoveAll(const std::filesystem::path& path) {
    ...
    if (ec && ec != std::errc::no_such_file_or_directory) {
        return std::unexpected(std::format("Failed to remove all: {} - {}",
                                          path.string(), ec.message()));
    }
    return {};
}
...
Result<void> StandardFileSystem::Copy(const std::filesystem::path& from, const std::filesystem::path& to) {
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to copy from {} to {} - {}",
...
Result<void> StandardFileSystem::Move(const std::filesystem::path& from, const std::filesystem::path& to) {
    ...
    if (ec) {
        ...
        std::filesystem::copy(from, to,
            ...
            ec);
        if (ec) {
            return std::unexpected(std::format("Failed to move from {} to {} - {}",
                                              from.string(), to.string(), ec.message()));
        }
        ...
        if (ec) {
            ...
            return std::unexpected(std::format("Warning: moved file but failed to remove source: {}",
                                              ec.message()));
        }
    }
    return {};
...
Result<std::filesystem::path> StandardFileSystem::GetCanonicalPath(const std::filesystem::path& path) {
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to get canonical path: {}", ec.message()));
    }
    return canonical;
}
...
Result<std::filesystem::path> StandardFileSystem::GetAbsolutePath(const std::filesystem::path& path) {
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to get absolute path: {}", ec.message()));
    }
    return absolute;
}

// Get relative path
Result<std::filesystem::path> StandardFileSystem::GetRelativePath(const std::filesystem::path& path, const std::filesystem::path& base) {
    std::error_code ec;
    auto relative = std::filesystem::relative(path, base, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to relative path to {} with {} - {}", path.string(), base.string(), ec.message()));
    }
    return relative;
}
...
Result<void> ExtendedFileSystem::AppendTextFile(const std::filesystem::path& path, std::string_view content) {
    ...
    if (!file) {
        return std::unexpected(GetStreamError(path, "Failed to append to file"));
    }
    
    return {};
}

Result<void> ExtendedFileSystem::AppendBinaryFile(const std::filesystem::path& path, std::span<const uint8_t> data) {
    errno = 0;  // Clear errno before operation
    std::ofstream file(path, std::ios::out | std::ios::binary | std::ios::app);
    if (!file) {
        return std::unexpected(GetStreamError(path, "Failed to open file for appending"));
...
Result<void> ExtendedFileSystem::WriteFileAtomic(const std::filesystem::path& path, std::string_view content) {
    ...
    if (ec) {
        ...
        return std::unexpected(std::format("Failed to atomically replace file: {}", ec.message()));
    }
...
Result<std::filesystem::space_info> ExtendedFileSystem::GetSpaceInfo(const std::filesystem::path& path) {
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to get space info: {}", ec.message()));
    }
    return space;
}

// Create temporary file
Result<std::filesystem::path> ExtendedFileSystem::CreateTempFile(const std::filesystem::path& directory,
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to get temp directory: {}", ec.message()));
    }
    ...
    if (!file) {
        return std::unexpected(GetStreamError(temp_path, "Failed to create temp file"));
    }
    file.close();
...
Result<std::filesystem::path> ExtendedFileSystem::CreateTempDirectory(const std::filesystem::path& directory,
                                                  const std::string& prefix) {
    std::error_code ec;
    std::filesystem::path dir = directory.empty() ? 
        std::filesystem::temp_directory_path(ec) : directory;
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to create temp directory: {}", ec.message()));
    }
...
Result<bool> ExtendedFileSystem::FilesEqual(const std::filesystem::path& path1, const std::filesystem::path& path2) {
    // Quick check: file sizes
    std::error_code ec;
    auto size1 = std::filesystem::file_size(path1, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to get size of first file: {}", ec.message()));
    }
    
    auto size2 = std::filesystem::file_size(path2, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to get size of second file: {}", ec.message()));
    }
...
Result<size_t> ExtendedFileSystem::ComputeSimpleHash(const std::filesystem::path& path) {
    ...
    if (!content) {
        return std::unexpected(std::format("Failed to read file for hashing: {}", content.error()));
    }
...
Result<std::filesystem::perms> ExtendedFileSystem::GetPermissions(const std::filesystem::path& path) {
    ...
    auto status = std::filesystem::status(path, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to get file permissions: {}", ec.message()));
    }
    return status.permissions();
}

// Set file permissions
Result<void> ExtendedFileSystem::SetPermissions(const std::filesystem::path& path, std::filesystem::perms perms) {
    std::error_code ec;
    std::filesystem::permissions(path, perms, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to set file permissions: {}", ec.message()));
    }
    return {};
}
...
// Get last write time
Result<std::filesystem::file_time_type> ExtendedFileSystem::GetLastWriteTime(const std::filesystem::path& path) {
    std::error_code ec;
    auto time = std::filesystem::last_write_time(path, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to get last write time: {}", ec.message()));
    }
    return time;
}

// Set last write time
Result<void> ExtendedFileSystem::SetLastWriteTime(const std::filesystem::path& path, 
                              std::filesystem::file_time_type time) {
    std::error_code ec;
    std::filesystem::last_write_time(path, time, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to set last write time: {}", ec.message()));
    }
    return {};
}
...
Result<void> ExtendedFileSystem::CreateSymlink(const std::filesystem::path& target, const std::filesystem::path& link) {
    std::error_code ec;
    std::filesystem::create_symlink(target, link, ec);
    if (ec) {
        return std::unexpected(std::format("Failed to create symbolic link: {}", ec.message()));
    }
    return {};
}
...
Result<std::filesystem::path> ExtendedFileSystem::ReadSymlink(const std::filesystem::path& path) {
    ...
    if (ec) {
        return std::unexpected(std::format("Failed to read symbolic link: {}", ec.message()));
    }
    return target;
}