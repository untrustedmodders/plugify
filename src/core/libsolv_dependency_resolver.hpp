...
namespace plugify {
	...
	private:
		...
		Id MakeDepConstraint(const std::string& name, const std::optional<Constraint>& constraints);
	...
	private:
		...
		Repo* _repo = nullptr;
		std::unordered_map<UniqueId, Id> _extensionToSolvableId;
		std::unordered_map<Id, UniqueId> _solvableIdToExtension;
	};
}  // namespace plugify