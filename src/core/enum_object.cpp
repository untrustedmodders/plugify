...
// Enum Implementation
EnumObject::EnumObject() : _impl(std::make_unique<Impl>()) {}
...
const std::string& EnumObject::GetName() const noexcept { return _impl->name; }
const std::vector<EnumValue>& EnumObject::GetValues() const noexcept { return _impl->values; }
void EnumObject::SetName(std::string name) { _impl->name = std::move(name); }
void EnumObject::SetValues(std::vector<EnumValue> values) { _impl->values = std::move(values); }
